let shuffle = function(){  
    ...
    different = true;                               //boolean to check whether the random number generated is
    i = 0;                                          
    ...
    while(i < 30){                                  // while loop to make sure it completes this process 30 times
        ...
        for(let index = 0; index< 30; index++){
            if(value === shuffled[index]){           // check to see if number has been used before
                different = false;                  // different set to false to identify that this is not a new number so don't add it to array
            }
        }
        if(different === true){
...
let button = document.querySelector('.cardbtn');        //variables used identifiers for html elements as easier to reffer to
